 ...
 */
  local G, sri, socreps, SF, Print, SA, nSF, genlist, projlist, pSQ,
        ...
        srit, msigens, msi, compsub, F, phi, reco, I, H, x, y, con, q,
        SQ, pSQH;
...
/*  end if; */
    ...
    reco:=rec<GR`RFG|>;
    if GR`presentation then
       reco`subgroup, reco`presentation := PresentationSubgroupTF
                            (newgenlist,newpreslist,GR`projlist,newfplist);
    else reco`subgroup := SubgroupTF(newgenlist);
    end if;
    reco`length := (#S div subgp`order)^#T;
    reco`order := #G div reco`length;
    AssertAttribute(reco`subgroup, "Order", reco`order);
    // assert not IsNormal(G,reco`subgroup);
    Append(~GR`maxsubs,reco);
    if Print gt 0 then
      print "  +Maximal subgroup of order",reco`order,
                                   "induced from almost simple group.";
      if Print gt 2 then print reco`subgroup; end if;
    end if;
  end for;
  return;
end procedure;
 ...
 */
  local G, M, sri, socreps, SF, nSF, Print, SA, projlist, reco, pSQ,
 ...
 */
  local G, sri, socreps, SF, nSF, Print, SA, projlist, reco,
 ...
 */
  local rho, W, G, eG, A, B, BG, M, SA, SB, SBG, NWA, CWA, NA, CA, pSA, SQ, pSQ,
        pNQ, NQ, T, pT, Comps, C, Ggens, Cgens, Qgens, Cgen, t, u, tail, reco,
...
// print "Type 4 pres";
  ...
  for comp in Comps do
    ...
    if GR`presentation then
       ...
       reco`subgroup, reco`presentation := PresentationSubgroupTF
                         (newgenlist,GR`preslist,GR`projlist,GR`fplist);
    else reco`subgroup := E @@ rho;
    end if;
    reco`order:=#reco`subgroup; 
...
/* Find maximal subgroups containing the socle, including the whole group. */
  local G, SF, nSF, pSQ, SQ, S, ms, newgenlist, reco, Print;
  ...
  if #SQ ne 1 then
    S := MaximalSubgroupsH(SQ,sub<SQ|>:Presentation:=GR`presentation,Print:=Print);
    for m in S do
      reco:=rec<GR`RFG|>;
      ms := m`subgroup;
      if GR`presentation then
// print "Type 5 pres";
      ...
      else reco`subgroup := ms@@pSQ;
      end if;
      reco`order := #G div Index(SQ,m`subgroup);
      AssertAttribute(reco`subgroup,"Order",reco`order);
...
PMaxSubsH := function(G,N:Print:=0,SSF:=5000,Presentation:=false)
