...
{ Find maximal subgroups of G.
  ...
  return MaxSubsH(G,sub<G|>:Print:=Print,SSF:=SmallSimpleFactor);
  end if;
end intrinsic;
...
WreathProductEmbeddings := procedure(~GR)
 ...
 */
  local rho, W, G, eG, A, B, BG, M, SA, SB, SBG, NWA, CWA, NA, 
	CA, pSA, SQ, pSQ,
        pNQ, NQ, T, pT, Comps, C, Ggens, Cgens, Qgens, Cgen, t, u, tail,
	reco,
  ...
  for comp in Comps do
    ...
    reco`length:=
	IsNormal(GR`group,reco`subgroup) select 1 
	    else Index(GR`group,reco`subgroup);
...
/* Find maximal subgroups containing the socle, including the whole group. */
  local G, SF, nSF, pSQ, SQ, S, newgenlist, reco, Print;
...
// finally do G.
	...
	reco`subgroup := G;
        ...
        reco`subgroup, reco`presentation := 
...
MaxSubsH := function(G,N:Print:=0,SSF:=5000)
/* Find maximal subgroups of G modulo the soluble normal subgroup N.
 * Use MaximalSubgroupsTF on the radical quotient where necessary.
 */
  local L, Q, pQ, M, MM, mm, s, Res;
  ...
  M := MaxSubsTF(Q,sub<Q|>:Print:=Print,SSF:=SSF);
  MM := [];
  Res := [];
  for m in M do
    mm := m;
