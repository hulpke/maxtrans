...
#  Global Variables used: AGL, Alt, AltSymMax, Append, AssertAttribute, Degree,
#  DirectProduct, Divisors, Factorial, ImprimitiveSubgroupsAltSym,
#  IntransitiveSubgroupsAltSym, IsAltsym, IsEven, IsPrime, Order, PGL, PSL,
#  PrimitiveGroup, PrimitiveSubgroupsAltSym, StandardGroup, Sym, WreathProduct

#  Defines: AltSymMax, ImprimitiveSubgroupsAltSym, IntransitiveSubgroupsAltSym,
#  MaximalSubgroupsAlt, MaximalSubgroupsAltSym, MaximalSubgroupsSym,
#  PrimitiveSubgroupsAltSym, RFG

#   Code for maximal subgroups of Sym(n)/Alt(n) taken from
#  * package/Group/GrpPerm/max/identify.m
#  * Main function is AltSymMax at end of file
...
local A,P,d,subs;
  ...
  for d in [1..QuoInt((n-1),2)] do
    if (sym) then
      P:=DirectProduct(SymmetricGroup(d),SymmetricGroup(n-d));
      AssertAttribute(P,"Order",Factorial(d)*Factorial(n-d));
      Add(subs,rec(subgroup:=P,
        order:=Order(P),
        length:=QuoInt(Factorial(n),Order(P))));
    else
      # rewritten select statement
      if d=1 then
        P:=SubStructure(SymmetricGroup(n),DirectProduct(AlternatingGroup(d)
         ,AlternatingGroup(n-d)));
      else
        P:=SubStructure(SymmetricGroup(n),DirectProduct(AlternatingGroup(d)
         ,AlternatingGroup(n-d)),#TODO CLOSURE
          Concatenation([2,1],[3..d],[d+2,d+1],[d+3..n]));
      fi;
      AssertAttribute(P,"Order",QuoInt(Factorial(d)*Factorial(n-d),2));
      Add(subs,rec(subgroup:=P,
        order:=Order(P),
        length:=QuoInt(Factorial(n),(2*Order(P)))));
    fi;
  od;
  return subs;
end;
...
local A,P,d,subs;
  ...
  for d in DivisorsInt(n) do
    if d<>1 and d<>n then
      ...
      P:=WreathProduct(SymmetricGroup(d),SymmetricGroup(QuoInt(n,d)));
      AssertAttribute(P,"Order",Factorial(d)^(QuoInt(n,d))*Factorial(QuoInt(n,d)
       ));
      if (sym) then
        Add(subs,rec(subgroup:=P,
          order:=Order(P),
...
local A,P,d,maxprim,subs,x;
  ...
  # rewritten select statement
  if sym then
    subs:=List(subs,s->rec(subgroup:=s,
      order:=Order(s),
      length:=QuoInt(Factorial(n),Order(s))));
  else
    subs:=List(subs,s->rec(subgroup:=s,
      order:=Order(s),
      length:=QuoInt(Factorial(n),(2*Order(s)))));
  fi;
  return subs;
end;
;
AltSymMax@:=function(n,sym)
#  /out:maximal subgroups of Sym(n) (sym=true) or Alt(n) (sym=false)
local subs;
  Assert(1,n >= 4);
  subs:=Concatenation(IntransitiveSubgroupsAltSym@(n,sym)
   ,ImprimitiveSubgroupsAltSym@(n,sym),PrimitiveSubgroupsAltSym@(n,sym));
  if sym then
    Add(subs,rec(subgroup:=AlternatingGroup(n),
      order:=QuoInt(Factorial(n),2),
      length:=1));
  fi;
  return subs;
end;
;
MaximalSubgroupsAltSym@:=function(n,sym)
#  -> ,SeqEnum  Maximal subgroups of the standard groups Alt ( n ) and Sym ( n
#  ) . Set sym true for Sym , false for Alt
if not n >= 4 then
    Error("degree must be at least 4");
  fi;
  return AltSymMax@(n,sym);
end;
;
MaximalSubgroupsAlt@:=function(n)
#  -> ,SeqEnum  Maximal subgroups of the standard groups Alt ( n ) .
if not n >= 4 then
    Error("degree must be at least 4");
  fi;
  return AltSymMax@(n,false);
end;
;
MaximalSubgroupsSym@:=function(n)
#  -> ,SeqEnum  Maximal subgroups of the standard groups Sym ( n ) .
if not n >= 4 then
    Error("degree must be at least 4");
  fi;
  return AltSymMax@(n,true);
end;
;
MaximalSubgroupsAltSym@:=function(G)
#  -> ,SeqEnum  Maximal subgroups of G , where G := assumed to be isomorphic to
#  either Alt ( n ) or Sym ( n ) , n ge 4 .
local S,f,i,maxsubs,n;
  n:=Degree(G);
  if not n >= 4 then
    Error("degree must be at least 4");
  fi;
  if IsAltsym(G) then
    maxsubs:=AltSymMax@(n,not IsEvenInt(G));
    # =v= MULTIASSIGN =v=
    f:=StandardGroup(G);
    S:=f.val1;
    f:=f.val2;
    ...
    for i in [1..Size(maxsubs)] do
      #  maxsubs[i]`subgroup := sub<G| (maxsubs[i]`subgroup)@@f>;
      maxsubs[i]:=maxsubs[i];
      #  (maxsubs[i]`subgroup)`Order := maxsubs[i]`order;
      maxsubs[i]:=maxsubs[i];
    od;
    return maxsubs;
  fi;
  Error("Unable to compute maximal subgroups");
end;
;
