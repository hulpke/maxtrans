...
function(L,q)
local 
   AC,AG,AI,C,CC,F,G,M,M2,OAG,Q,_,a,c,co,continue,d,det,dsq,f,ff,form,forms,
   g,general,gens,got,gps,i,isit,iso,j,lv,mat,modims,na,nc,new,normaliser,o,
   ox,perms,phi,po,quad,rq,rts,sAG,scal,sgens,sgn,special,stuff,tmat,u,v,w,
   ww,x;
  ...
  #  *
  #  * By default, the subgroup of the relevant quasisimple group is
  #  returned.
  #  * If special is true (case O only), normaliser in SO is returned.
  #  * If general is true (cases L/U/O), normaliser in GL, GU, GO is
  #  returned.
  ...
  Q:=Group(G.1,G.2);
  M:=GModuleByMats(GeneratorsOfGroup(Q),DefaultFieldOfMatrixGroup(Q));
  C:=MTX.CompositionFactors(M);
  CC:=[];
  for c in C do
  ...
  for o in OAG do
    ...
    if forms.formType="linear" then
      ww:=SELECT(general then w else w^(QuoInt((q-1),Gcd(q-1,d))));
      ...
      for j in sgens do
        ...
        # =^= MULTIASSIGN =^=
        if isit then
          iso:=iso*ScalarMatrix(GF(q),d,v^-1);
          ...
          for i in [1..Size(f)] do
            ...
            if rts<>[] then
              ...
              if general and not got then
                iso:=iso*ScalarMatrix(GF(q),d,rts[1]^-1);
              fi;
            fi;
          od;
        fi;
      ...
      if (not general and Gcd(q-1,d)<>1) or (general and q > 2) then
        Add(gens,ScalarMatrix(GF(q),d,ww));
      fi;
      ...
      ww:=SELECT(normaliser then w else SELECT(general then w^(rq-1) else 
       w^((rq-1)*(QuoInt((rq+1),Gcd(rq+1,d))))));
      ...
      for j in sgens do
        ...
        scal:=scal;
        # =v= MULTIASSIGN =v=
        v:=IsPower(scal*FORCEOne(GF(q)),rq+1);
        isit:=v.val1;
        v:=v.val2;
        # =^= MULTIASSIGN =^=
        ...
        #  We will try to make the order of iso as small as possible.
        #  if general is true, we will sacrifice determinant 1 if necessary
        #  if normaliser is true, we will sacrifice fixing form if 
         necessary
        # =v= MULTIASSIGN =v=
        x:=ProjectiveOrder(iso);
        po:=x.val1;
      ...
      for j in sgens do
        ...
        if quad then
          ...
          for i in [2..d] do
            for j in [1..i-1] do
              ...
              if mat[j][i]<>0*FORCEOne(GF(q)) and not got then
        ...
        if isit then
          ...
          continfi;
        det:=DeterminantMat(iso);
        if det=(-1)*FORCEOne(GF(q)) and IsOddInt(d) then
          iso:=iso*ScalarMatrix(GF(q),d,(-1)*FORCEOne(GF(q)));
        elif not general and det<>1*FORCEOne(GF(q)) then
          continfi;
        iso:=tmat^-1*iso*tmat;
        if not special then
          if (forms.formType="orthogonalplus" and not InOmega@(iso,d,q,1)) 
             or (forms.formType="orthogonalminus" and not 
             InOmega@(iso,d,q,-1)) then
            if IsOddInt(q) and not dsq then
              iso:=-iso;
            else
              continfi;
          fi;
          if forms.formType="orthogonalcircle" and not InOmega@(iso,d,q,0) 
             then
            continfi;
        fi;
        #  We will try to make the order of iso as small as possible.
        ...
        if special and ox > 1 then
          ...
          for i in [1..Size(f)] do
            ...
            if rts<>[] then
              got:=false;
              for v in rts do
                if v=(-1)*FORCEOne(GF(q)) and IsEvenInt(d) and IsOddInt(q) 
                   then
                  #  v * I_d has det 1
                  iso:=iso*ScalarMatrix(GF(q),d,v^-1);
                  got:=true;
      ...
      if normaliser and q > 2 then
        Add(gens,ScalarMatrix(GF(q),d,w));
      elif (general and IsOddInt(q)) or (special and IsEvenInt(d) and 
         IsOddInt(q)) or (forms.formType="symplectic" or (IsEvenInt(d) and 
         dsq)) then
        Add(gens,ScalarMatrix(GF(q),d,(-1)*FORCEOne(GF(q))));
      fi;
      Add(gps,SubStructure(GL(d,q),gens));
    fi;
  od;
