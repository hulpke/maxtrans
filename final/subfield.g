...
local c,divisor,general,isc,mat,new_gen,new_scal,newgens,prim_scal,sol,vec,z;
  ...
  if general then
    newgens:=GeneratorsOfGroup(GL(d,p^f)){[1,2]};
  else
    newgens:=GeneratorsOfGroup(SL(d,p^f)){[1,2]};
  fi;
  z:=PrimitiveElement(GF(p^e));
  if general then
    return SubgroupContaining(GL(d,p^e),newgens,
      z*IdentityMat(d,GF(q)));
  fi;
  divisor:=Gcd(p^e-1,d);
  ...
  if c mod Gcd(p^f-1,d)=0 then
    return SubgroupContaining(SL(d,p^e),newgens,prim_scal);
  fi;
  mat:=MatrixByEntries(Integers,2,1,[d,p^e-1]);
  vec:=[QuoInt(c*(p^e-1),(p^f-1))]l
  ...
  #  - 1) div divisor) eq 0};
  #  new_scal:= ScalarMat(d, z^-t);
  
  new_scal:=ScalarMat(d,z^-sol[1]);
  new_gen:=new_scal*DiagonalMat(Concatenation([z^(QuoInt((p^e-1),(p^f-1)))]
   ,List([2..d],i->Z(q)^0)))^c;
  Assert(1,IsOne(DeterminantMat(new_gen)));
  return SubgroupContaining(GL(d,p^e),newgens,prim_scal,new_gen);
end);
...
local delta,newgens,norm,normaliser,norms,power,scal_power,z;
  ...
  Assert(1,IsPrimeInt(p));
  Assert(1,e mod f=0);
  newgens:=GeneratorsOfGroup(SP(d,p^f)){[1,2]};
  z:=PrimitiveElement(GF(p^e));
  norms:=NormSpMinusSp@(d,p^f);
  if normaliser then
    return SubgroupContaining(GL(d,p^e),newgens,norms,ScalarMat(d,z));
  fi;
  ...
  Add(newgens,delta*ScalarMat(d,z^-scal_power));
...
local 
  ...
  # rewritten select statement
  if general then
    newgens:=GeneratorsOfGroup(GU(d,p^f)){[1,2]};
  else
    newgens:=GeneratorsOfGroup(SU(d,p^f)){[1,2]};
  fi;
  z:=PrimitiveElement(GF(p^(2*e)));
  if normaliser then
    return SubgroupContaining(GL(d,p^(2*e)),newgens,ScalarMat(d,z));
  elif general then
    return SubgroupContaining(GL(d,p^(2*e)),newgens,ScalarMat(d,z^(p^e-1)));
  fi;
  divisor:=Gcd(p^e+1,d);
  c:=QuoInt((divisor*Lcm(p^f+1,QuoInt((p^e+1),divisor))),(p^e+1));
...
local 
  ...
  #  ];
  newgens:=GeneratorsOfGroup(SP(d,q)){[1,2]};
  ...
  #  m:= Gcd(q+1, d);
  #  Append(~newgens, ScalarMat(d, z^((q^2-1) div m)));
  #  end if;
...
local 
  ...
  z:=PrimitiveElement(GF(q^2));
  prim_scal_su:=ScalarMat(d,z^(QuoInt((q^2-1),Gcd(q+1,d))));
  prim_scal_gu:=ScalarMat(d,z^(q-1));
  prim_scal:=ScalarMat(d,z);
  if IsOddInt(d) then
    #newgens:=[Eltseq(SO(d,q).1) #CAST GL(d,q^2)
    #  ,Eltseq(SO(d,q).2) #CAST GL(d,q^2)
    #  ];
    newgens:=GeneratorsOfGroup(SO(d,q)){[1,2]};
    newgrp:=Group(newgens);
    # =v= MULTIASSIGN =v=
    ...
    Assert(1,isit);
    if normaliser then
      newgrp:=SubgroupContaining(GL(d,q^2),newgens, prim_scal);
    elif general then
      newgrp:=SubgroupContaining(GL(d,q^2),newgens, prim_scal_gu);
    else
      newgrp:=SubgroupContaining(GL(d,q^2),newgens, prim_scal_su);
    fi;
  ...
  # rewritten select statement
  if general then
    newgens1:=GeneratorsOfGroup(GOPlus(d,q));
  else
    newgens1:=GeneratorsOfGroup(SOPlus(d,q));
  fi;
  # rewritten select statement
  if general then
    newgens2:=GeneratorsOfGroup(GOMinus(d,q));
  else
    newgens2:=GeneratorsOfGroup(SOMinus(d,q));
  fi;
  if IsEvenInt(q) then
    return rec(val1:=Group(newgens1),
      val2:=Group(newgens2));
  fi;
  # =v= MULTIASSIGN =v=
  ...
  Assert(1,UnitaryForm@(SubgroupContaining(GL(d,q^2),newgens1, y_plus)));
  y_minus:=delta_minus*prim_scal^-1;
  Assert(1,UnitaryForm@(SubgroupContaining(GL(d,q^2),newgens2, y_minus)));
